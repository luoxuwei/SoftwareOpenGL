...
Mesh::Mesh(
	...
	mVao = glGenVertexArray();
	mVbo = glGenBuffer();
	mEbo = glGenBuffer();

	glBindBuffer(ARRAY_BUFFER, mVbo);
	glBufferData(ARRAY_BUFFER, vertices.size() * sizeof(Vertex), (void*) & vertices[0]);

	glBindBuffer(ELEMENT_ARRAY_BUFFER, mEbo);
	glBufferData(ELEMENT_ARRAY_BUFFER, indices.size() * sizeof(uint32_t), (void*)&indices[0]);
	mIndicesCount = indices.size();

	glBindVertexArray(mVao);
	glVertexAttributePointer(0, 3, sizeof(Vertex), 0);
	glVertexAttributePointer(1, 3, sizeof(Vertex), sizeof(float) * 3);
	glVertexAttributePointer(2, 2, sizeof(Vertex), sizeof(float) * 6);
	glBindVertexArray(0);
}

Mesh::~Mesh() {
	if (mVao) {
		glDeleteVertexArray(mVao);
	}
	if (mVbo) {
		glDeleteBuffer(mVbo);
	}
...
void Mesh::draw(const math::mat4f& preMatrix, LightShader* shader) {
	...
	if (mVao) {
		...
		glBindVertexArray(mVao);
		glBindBuffer(ELEMENT_ARRAY_BUFFER, mEbo);
		glDrawElement(DRAW_TRIANGLES, 0, mIndicesCount);
		glBindVertexArray(0);
	}
